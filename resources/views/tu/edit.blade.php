...
@section('content')
    ...
    <div class="title-wrapper pt-30">
        <div class="row align-items-center">
            <div class="title">
                <h2 class="text-2xl font-semibold text-gray-800">Upload Surat</h2>
            </div>
            <!-- end col -->
        </div>
        <!-- end row -->
        ...
        <div class="select-style-1">
            ...
            <div class="select-position">
                <select name="jenis_surat_id" class="text-capitalize" required readonly>
                    <option value="{{ $detailSurat->surat->jenisSurat->id }}" selected>
                        {{ $detailSurat->surat->jenisSurat->name }}</option>
                </select>
            </div>
            ...
            <form method="POST" action="{{ route('tu.upload', $detailSurat->surat_id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="input-style-1">
                    <label>Upload Surat</label>
                    <input type="file" name="fileSurat" accept="application/pdf" required/>
                </div>
